...
# 💡 국내외 Ticker 및 지수 매핑 데이터 정밀화
ticker_map = {
    "KODEX 200 (069500)": {"etf": "069500.KS", "nav_proxy": "^KS11"},           # KOSPI 200 지수 추적
    "삼성전자 (005930)": {"etf": "005930.KS", "nav_proxy": "^KS11"},            # 코스피 대장주 vs 시장지수
    "TIGER 반도체TOP10 (396500)": {"etf": "396500.KS", "nav_proxy": "^KRXSEM"}, # KRX 반도체 지수 추적
    "PLUS 글로벌HBM반도체 (442580)": {"etf": "442580.KS", "nav_proxy": "^KRXSEM"},
    "KODEX 미국반도체 (390390)": {"etf": "390390.KS", "nav_proxy": "^SOX"},      # 필라델피아 반도체 지수(^SOX) 추적
...
while True:
    with placeholder.container():
        try:
            ...
            if "069500" in etf_ticker:  # KODEX 200 -> KOSPI 지수와 동기화 시뮬레이션
                ratio = (etf_price * 10) / base_nav
            elif "005930" in etf_ticker: # 삼성전자 -> 시장 영향도 시뮬레이션
            ...
            st.warning("시장 마감 시간(휴일)이거나 데이터를 동기화 중입니다.")
